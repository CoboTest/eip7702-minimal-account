...
- ZeroDev：（`ZERODEV_PROJECT_ID` 或 `ZERODEV_BUNDLER_RPC`）+ 可选 `ZERODEV_PAYMASTER_RPC` 覆盖
- thirdweb：（`THIRDWEB_BUNDLER_URL` 或基于 CHAIN_ID 自动推导），以及（`THIRDWEB_SECRET_KEY` 或 `THIRDWEB_CLIENT_ID`）
...
- **返回：** `transactionHash`、`blockNumber`、`success`
