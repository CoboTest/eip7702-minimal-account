...
- 🌐 [External Provider Comparison](test-reports/e2e-external-providers-en.md) — Pimlico vs Alchemy vs ZeroDev (RPC/response/on-chain results)
...
- Python ZeroDev flow: (`ZERODEV_PROJECT_ID` or `ZERODEV_BUNDLER_RPC`) + optional `ZERODEV_PAYMASTER_RPC` override
- Python thirdweb flow: (`THIRDWEB_BUNDLER_URL` or CHAIN_ID-derived default) + (`THIRDWEB_SECRET_KEY` or `THIRDWEB_CLIENT_ID`)
