"""Alchemy bundler service (Rundler-backed ERC-4337 endpoints)."""
...
class AlchemyBundler(AlchemyJsonRpcMixin, Bundler):
    """Alchemy bundler implementation.

    Uses standard ERC-4337 methods plus `rundler_maxPriorityFeePerGas`.
    """
    ...
    async def get_gas_price(self) -> GasPrice:
        # Alchemy-specific method for priority fee guidance.
        priority_hex = await self._rpc("rundler_maxPriorityFeePerGas", [])
        priority = int(priority_hex, 16)

        # Base fee proxy from rpc gas price; derive a conservative maxFee.
    ...
    def __repr__(self) -> str:
        return f"AlchemyBundler(ep={self._entry_point})"
