...
> 本分支使用 **EntryPoint v0.7**。如需 EntryPoint v0.8（原生 EIP-7702 支持），请查看 `feature/entrypoint-v08` 分支。
...
└─────────────────────────────────────────┘
   ...
   ERC-7821 接口         → EIP-191 + ECDSA == address(this)
```
...
- Python ZeroDev 流程：（`ZERODEV_PROJECT_ID` 或 `ZERODEV_BUNDLER_RPC`）+ 可选 `ZERODEV_PAYMASTER_RPC` 覆盖
- Python thirdweb 流程：（`THIRDWEB_BUNDLER_URL` 或基于 CHAIN_ID 自动推导）+（`THIRDWEB_SECRET_KEY` 或 `THIRDWEB_CLIENT_ID`）
